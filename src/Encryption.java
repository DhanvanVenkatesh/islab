...
public class Encryption extends JFrame {
	...
	public Encryption() {
		...
		JLabel lblTemperatureConversion = new JLabel("Encryption");
		...
		btnC.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e1) {
				...
				else 
				{
					...
					BigInteger e,n;
					String pt, ans="";
			        e = new BigInteger(c.getText());
			        n = new BigInteger(f.getText());
			        pt = k.getText();
			        int len = pt.length();
			        BigInteger val[] = new BigInteger[len];
			        for(int i=0; i<pt.length(); i++){
			            BigInteger a = BigInteger.valueOf(pt.charAt(i));
			            val[i] = expo(a,e,n);
			        }
				    for(int i=0; i<pt.length(); i++){
			            ans += String.valueOf(Character.toChars(val[i].intValue()));
				    }
					tfOut.setText(ans);
				}
